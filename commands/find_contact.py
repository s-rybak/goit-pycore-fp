...
class FindContactCommand(CommandInterface):
    ...
    def execute(self, input: InputInterface, output: OutputInterface, args: list):
        if len(args) < 2:
            output.display_message(Message("Usage: find <field> <value>. Fields: name, phone, email, address, birthday"))
            return
        ...
        if field not in search_methods:
            output.display_message(Message("Invalid field. Use: name, phone, email, address or birthday"))
            return
